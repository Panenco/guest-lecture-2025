...
const templateMap: Record<CommunicationKey, { bodyPath: string; subject: string }> = {
  [CommunicationKey.temperatureWarning]: {
...
export interface CommunicationRequestBody<Key extends CommunicationKey = CommunicationKey> {
...
export class EmailService {
  ...
  async sendEmail<T extends CommunicationKey>({ model, key, recipient: to }: CommunicationRequestBody<T>) {
    const templates = templateMap[key];
    const otherFiles = fs.readdirSync(`${__dirname}/templates`);
    console.log(otherFiles);
    const bodyTemplate = fs.readFileSync(`${__dirname}/templates/${templates.bodyPath}`, 'utf8');
